...
function pointAtDistance(array, distance) {
  ...
  for (i = 0; i + 1 < array.length; i++) {
    var nd = distancePoints(array[i], array[i + 1])
    if (current_distance + nd >= distance) {
      break
    }
    current_distance += nd;
  }
  if (i === array.length - 1)
    return [array[i][0], array[i][1], angleFromPoints(array[i - 1], array[i]), i - 1];
  var angle = angleFromPoints(array[i], array[i + 1]);
  var cart = cartFromPol(distance - current_distance, angle);
  var ans;
  if (array[i][0] > array[i + 1][0]) {
    return [array[i][0] - cart[0], array[i][1] - cart[1], angle, i];
  }
  return [array[i][0] + cart[0], array[i][1] + cart[1], angle, i];
}
